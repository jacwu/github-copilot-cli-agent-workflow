...
3. Call POST /repos/{owner}/{repo}/pulls
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Create a pull request using the GitHub REST API"
    )
...
def main() -> int:
    ...
    try:
        ...
        if exc.code == 401:
            ...
            print(f"Error: Repository {owner}/{repo} not found, or token lacks access", file=sys.stderr)
        elif exc.code == 422:
            print("Error: Invalid request. Branch may not exist, PR may already exist, or there are no commits between branches", file=sys.stderr)
        else:
    ...
    print(f"Created PR #{result.get('number', '?')}")
